...
struct Classtable* CInstall(char *name, char *parentClassName, struct Classtable *Cptr) {
    ...
    if (parentClassName == NULL) {
        ...
        if (Ctemp->parentPtr == NULL) {
            fprintf(stderr, "Error: No class named '%s'.\n", parentClassName);
            exit(1);
        }
        Ctemp->fieldPtr = Ctemp->parentPtr->fieldPtr;
        struct Methodlist *parentMethod;
        struct Methodlist **lastMethodNode;
        lastMethodNode = &(Ctemp->methodPtr);
        parentMethod = Ctemp->parentPtr->methodPtr;
        while(parentMethod != NULL) {
            struct Methodlist *Mtemp = (struct Methodlist*)malloc(sizeof(struct Methodlist));
            Mtemp->name = strdup(parentMethod->name);
            Mtemp->type = parentMethod->type;
            Mtemp->paramList = parentMethod->paramList;
            Mtemp->funcIndex = parentMethod->funcIndex;
            Mtemp->flabel = strdup(parentMethod->flabel);
            Mtemp->next = NULL;
            *lastMethodNode = Mtemp;
            lastMethodNode = &(Mtemp->next);
...
void Class_MInstall(char *name, struct Typetable *type, struct Paramstruct *paramList, struct Classtable *Cptr) {
    ...
    if (Class_MLookup(name, Cptr) != NULL) {
        struct Classtable *parent = Cptr->parentPtr;
        while(parent!=NULL) {
            if (Class_MLookup(name, parent) != NULL) {
                break;
            }
...
void initFuncTable() {
    ...
    Ctemp = Chead;
    while(Ctemp != NULL) {
        int mem = 4096 + 8 * Ctemp->classIndex;
        struct Methodlist *Mtemp = Ctemp->methodPtr;
        while(Mtemp != NULL) {
            int binding = mem + Mtemp->funcIndex;
            if (Mtemp->flabel == NULL) {
                fprintf(stderr, "Error: Method '%s' is not defined within '%s' class.\n", Mtemp->name, Ctemp->name);
                exit(1);
            }
            fprintf(target_file, "MOV [%d], <%s>\n", binding, Mtemp->flabel);
            Mtemp = Mtemp->next;
        }
        Ctemp = Ctemp->next;
    }
