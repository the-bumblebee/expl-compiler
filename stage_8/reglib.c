...
int getIDAddress(struct tnode *t) {
    ...
    if (t->nodetype == 30) {
        struct tnode *temp = t->left->left;
//EXPERIMENTAL
        if (temp->nodetype == 29) {
            if(temp->left == NULL)  {
                fprintf(target_file, "MOV R%d, BP\n", p_reg);
                fprintf(target_file, "ADD R%d, -3\n", p_reg);
                fprintf(target_file, "MOV R%d, [R%d]\n", p_reg, p_reg);
                return p_reg;
            }
            else if(temp->left != NULL) {
                struct Fieldlist *Flist;
                Flist = Class_Flookup(temp->left->name, temp->ctype);
                fprintf(target_file, "MOV R%d, BP\n", p_reg);
                fprintf(target_file, "ADD R%d, -4\n", p_reg);
                fprintf(target_file, "MOV R%d, [R%d]\n", p_reg, p_reg);
                fprintf(target_file, "ADD R%d, %d\n", p_reg, Flist->fieldIndex + 1);
                fprintf(target_file, "MOV R%d, [R%d]\n", p_reg, p_reg);
                return p_reg;
            }
//EXPERIMENTAL
            ...
            if (Gentry == NULL) {
                fprintf(stderr, "Compiler Error: nodetype 30, left->Gentry = NULL\n", temp->name);
                exit(1);
            }
    ...
    //Check for ID node
    else  if ((t->name == NULL || t->nodetype == 22 || t->nodetype == 23) && (t->nodetype != 24)) {
        ...
        switch (t->nodetype) {
            ...
            case 5:     q_reg = codeGen(t->right);
                        ...
                        if (t->left->ctype != NULL) {
                            if (t->right->nodetype == 25) {
                                p_reg = getIDAddress(t->left);
                                int vptr = 4096 + 8 * (t->right->ctype->classIndex);
                                fprintf(target_file, "ADD R%d, 1\n", p_reg);
                                fprintf(target_file, "MOV [R%d], %d\n", p_reg, vptr);
                                freeReg();
                            }
            ...
            case 30:    used_reg = reg_count;
                        ...
                        //Puushing self/Obj address
                        {
                            ...
                            fprintf(target_file, "PUSH R%d\n", addrReg);
                            freeReg();
                            addrReg = getIDAddress(t);
                            fprintf(target_file, "PUSH R%d\n", addrReg);
                            //Allocating a space in stack to store return value.
                            fprintf(target_file, "PUSH R0\n");
                            fprintf(target_file, "ADD R%d, %d\n", addrReg, Class_MLookup(t->right->name, t->left->ctype)->funcIndex);
                            fprintf(target_file, "MOV R%d, [R%d]\n", addrReg, addrReg);
                            //Calling the function.
