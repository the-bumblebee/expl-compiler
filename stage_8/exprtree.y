...
%token RETURN MAIN NUM COL PLUS MINUS MUL DIV MOD END BEGINI READI WRITEI ID EQUAL IF then Else ENDIF WHILE DO ENDWHILE BREAK CONTINUE STRLIT DECL ENDDECL INT STR ERROR TYPE ENDTYPE ALLOC FREE NULLV INITIALIZE CLASS ENDCLASS SELF NEW DELETE EXTENDS
%type <number> NUM
%type <name> ID
%type <literal> STRLIT
...
Cname			: ID       		{Chead = CInstall($1 ,NULL, Chead);}
				| ID EXTENDS ID	{Chead = CInstall($1, $3, Chead);}
				;
