...
int getIDAddress(struct tnode *t) {
    //Check for ID node
    if (t->varname == NULL || t->nodetype == 22 || t->nodetype == 23) {
...
int codeGen(struct tnode *t) {
    ...
    //Filtering array nod(19), function definiton node(22) and function caller node out(23).
    else if (t->type == inttype && (t->nodetype != 19 && t->nodetype != 22 && t->nodetype != 23)) {
    ...
    //Statements
        ...
        switch (t->nodetype) {
            ...
            //func declaration
            case 22:    fprintf(target_file, "L%d:\n", t->Gentry->flabel);
                        if (t->Gentry->flabel != 0)
